...
| `Standorte` | gated (`RequireAuth`) | Read-only "info center": location list (photo card + address preview) → detail (address + "get directions" via a Google Maps search URL, opening hours, contact, access note, key-holder count, WiFi networks with password hidden behind a "show QR code" toggle that renders a `react-native-qrcode-svg` code via `buildWifiQrPayload`, links). Create/edit/delete lives on the separate admin website, out of scope here |
