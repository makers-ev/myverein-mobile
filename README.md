...
| `Standorte` | gated (`RequireAuth`) | Tabs: Orte (read-only "info center" — location list (photo card + address preview) → detail: address + "get directions" via a Google Maps search URL, opening hours, contact, access note, key-holder count, WiFi networks with password hidden behind a "show QR code" toggle that renders a `react-native-qrcode-svg` code via `buildWifiQrPayload`, links), Material (self-service inventory — item list with a condition badge and a `themeColors.warning`-tinted "maintenance due" badge → detail: borrow/return (self-service loan, own open loan resolved via `useOwnMembership`), file a damage report with an optional photo via `expo-image-picker` + `src/lib/media.ts`'s `uploadMedia`/`downloadMediaUri` (the latter needed because `GET /media/:key` requires the session cookie, so photos are fetched authenticated and cached to a local `file://` URI for `<Image>`)). Item/location/WiFi/link create/edit/delete all live on the separate admin website, out of scope here |
