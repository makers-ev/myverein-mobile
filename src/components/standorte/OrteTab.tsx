 ...
 * navigator route for the detail view). Extracted unchanged from
 * `StandorteScreen.tsx` when the "Material" tab was added alongside it.
 */
export default function OrteTab({ clubId }: { clubId: string }) {
  const [selectedLocationId, setSelectedLocationId] = useState<string | null>(null);

  return selectedLocationId ? (
    <LocationDetail clubId={clubId} locationId={selectedLocationId} onBack={() => setSelectedLocationId(null)} />
  ) : (
    <LocationList clubId={clubId} onSelect={setSelectedLocationId} />
  );
}
